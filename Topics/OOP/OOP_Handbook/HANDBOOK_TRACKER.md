...
- **Four Pillars of OOP at a high level** — Chapter 1, §1.8. (Encapsulation: Ch. 12. Inheritance: Ch. 15. Polymorphism: Ch. 16 — all now done. Abstraction still gets its full chapter at Ch. 17.)
...
- **Method signature (excludes return type), overload resolution at compile time, the full pass-by-value proof (mutate-through-reference vs. reassign-reference), stack frames per method call** — Chapter 7, §2–§6. (`invokevirtual`/dynamic dispatch itself now fully explained in Ch. 16, §4.)
...
- **Packages — namespace + mandatory directory mapping, fully-qualified names, imports (incl. the wildcard-import performance myth debunked), `java.lang`'s implicit import, static imports, and how the classloader maps a fully-qualified name to a classpath search** — Chapter 14, §2–§4.
- **Inheritance — the is-a test, what's inherited vs. never inherited vs. present-but-inaccessible, method overriding rules, `@Override`, overloading/overriding/hiding as three distinct mechanisms, single inheritance and the diamond problem, the three inheritance shapes, `Object` as implicit universal parent, and the `super_class` constant-pool reference** — Chapter 15, §2–§8.
- **Dynamic method dispatch (the vtable mechanism behind `invokevirtual`), compile-time vs. runtime polymorphism fully contrasted, upcasting, `instanceof`/downcasting (incl. Java 16+ pattern matching), and — critically — that fields are never polymorphic in Java, always resolved by declared reference type** — Chapter 16, §2–§6. Definitive; only reference, don't re-derive.

## Next Up

➡️ Chapter 17 — Abstraction
