...
- **`Object`'s default `toString()`/`equals()`/`hashCode()`, correct override patterns for all three, the full equals() contract, the equals/hashCode pairing requirement and the concrete `HashSet` failure mode when it's violated, `Objects.equals()`/`Objects.hash()`, `getClass()` vs. `instanceof` in `equals()`, and why identity fields should be immutable** — Chapter 19, §3–§9. Definitive; only reference, don't re-derive. (`clone()`/deep vs. shallow copy still belongs fully to Ch. 29; `getClass()`/reflection in depth still belongs to Ch. 30.)

## Next Up

➡️ Chapter 20 — Association (begins Part X: Object Relationships)
