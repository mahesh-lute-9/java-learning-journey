...
- **Four Pillars of OOP at a high level** — Chapter 1, §1.8. (Encapsulation: Ch. 12. Inheritance: Ch. 15, done. Polymorphism, Abstraction still get theirs at Ch. 16, 17.)
...
- **`static` methods (no `this`, no direct instance access, `invokestatic`, resolved at compile time), class-name-vs-instance calling style, static utility classes, method hiding vs. overriding, unified static-variable/static-block/static-method picture** — Chapter 9, §2–§7. (Static nested classes still belong fully to Ch. 23.)
...
- **`super()` constructor-call rules (incl. the no-accessible-no-arg-parent gotcha), the complete initialization order once a parent class exists, `super.field`/`super.method()`, and why `super.method()` uses static (`invokespecial`) resolution rather than dynamic dispatch** — Chapter 11, §2–§4.
...
- **All four access levels (`private`, package-private/default, `protected`, `public`), the package-private-vs-private distinction, `protected`'s cross-package subclass-own-type rule, and the `ACC_*` class-file flag mechanism** — Chapter 13, §2–§7.
- **Packages — namespace + mandatory directory mapping, fully-qualified names, imports (incl. the wildcard-import performance myth debunked), `java.lang`'s implicit import, static imports, and how the classloader maps a fully-qualified name to a classpath search** — Chapter 14, §2–§4. (Full classloader mechanics, multiple classloaders, and delegation — still belong to Ch. 39.)
- **Inheritance — the is-a test (vs. has-a/Composition), what's inherited vs. never inherited (constructors) vs. present-but-inaccessible (`private` fields), method overriding rules (signature, covariant return, visibility, `final`), `@Override` as compile-time-only, overloading/overriding/hiding as three distinct mechanisms, single inheritance and the diamond problem, the three inheritance shapes, `Object` as implicit universal parent, and the `super_class` constant-pool reference** — Chapter 15, §2–§8. Definitive for everything except *how* an overridden method is chosen at runtime — that mechanism (dynamic dispatch, virtual method tables) is entirely Chapter 16's job, deliberately not touched here.

## Next Up

➡️ Chapter 16 — Polymorphism
