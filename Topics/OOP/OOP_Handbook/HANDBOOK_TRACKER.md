...
- **Four Pillars of OOP** — Chapter 1, §1.8 (preview); all four now fully covered: Encapsulation (Ch. 12), Inheritance (Ch. 15), Polymorphism (Ch. 16), Abstraction (Ch. 17). Java's full abstraction toolkit (abstract classes + interfaces) complete as of Ch. 18.
...
- **Interfaces — implicit `public`/`abstract` methods and `public static final` fields, multiple implementation, default methods (Java 8+) and their backward-compatibility motivation, the partial diamond-problem return (resolved by forced explicit override), `invokeinterface` as a distinct dispatch mechanism from `invokevirtual`, abstract-class-vs-interface decision table** — Chapter 18, §2–§5. Definitive; only reference, don't re-derive.

## Next Up

➡️ Chapter 19 — The Object Class (begins Part IX)
