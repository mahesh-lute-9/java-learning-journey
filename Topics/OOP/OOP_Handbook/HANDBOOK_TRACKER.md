...
- **Anonymous classes — nameless local classes instantiated inline, no explicit constructor (instance initializer block instead), exactly one supertype (never both a class and an interface), identical effectively-final capture rule, sequential `$N` compiler naming, and the precise anonymous-class-vs-lambda `this`-binding distinction** — Chapter 24, §2–§5. Definitive; only reference, don't re-derive. (Lambdas and functional interfaces themselves remain outside this handbook's OOP scope, mentioned only for the `this` contrast.)
