...
- **Instance vs. static vs. local variables — memory location, default-value behaviour, scope, shadowing, `var` type inference** — Chapter 4, §2–§7. (Local-variable Stack lifetime now directly explains Ch. 23 §6.1's effectively-final capture rule.)
...
- **`this` — implicit object reference, shadowing fix, passing/returning `this`, unavailable in static context, invisible-first-parameter bytecode fact** — Chapter 8, §2–§6. (Directly parallels Ch. 23 §5's `this$0` for inner classes.)
- **`static` methods, class-name-vs-instance calling style, static utility classes, unified static-variable/static-block/static-method picture, static nested classes preview** — Chapter 9, §2–§7. Static nested classes now fully covered in Ch. 23.
...
- **Nested classes — static nested classes (no enclosing-instance reference) vs. inner classes (implicit `this$0` reference to a specific enclosing instance), the `outer.new Inner()` syntax, local classes and the effectively-final capture rule (compiler copies captured values at creation time, tied directly to Ch4's Stack lifetime model)** — Chapter 23, §2–§6. Definitive; only reference, don't re-derive. (Anonymous classes — the fourth nesting form — belong entirely to Ch. 24.)

## Next Up

➡️ Chapter 24 — Anonymous Classes
